 ...
 */
export function useFlowData({ tokens, chains, active, mode }: { tokens: string[]; chains: string[]; active: boolean; mode: FlowViewMode }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/flow?tokens=${encodeURIComponent(tkKey)}&chains=${encodeURIComponent(chKey)}&topPct=0.999&maxProto=28&maxMkt=30`, { cache: "no-store" })
      .then((r) => r.json())
