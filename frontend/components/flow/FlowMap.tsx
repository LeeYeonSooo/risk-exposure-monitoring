...
/**
 * 흐름맵 본체(컨트롤바 + 그래프 + 디테일) — 페이지 크롬(SiteHeader/AlertDock) 없이 어디든 임베드.
 * 메인 페이지(흐름맵 + 모니터링 2분할)와 /flow(전체화면) 양쪽에서 재사용. 부모 flex 영역을 채운다.
 *  · 실시간 — 최근 30분(1분 지연) 실전송이 입자로 엣지를 탄다 (크기 ∝ 금액)
 *  · 평소   — 최근 24h 집계로 "어느 엣지로 얼마나 흐르는 게 평소인가"를 두께·수치로
 * ?token=SYMBOL preselects a token (e.g. linked from a token page).
 */
export function FlowMap() {
  return (
    <Suspense fallback={<div className="flex min-h-0 flex-1 items-center justify-center text-sm text-[var(--color-text-muted)]">흐름맵 로딩…</div>}>
      <FlowMapInner />
    </Suspense>
  );
}
