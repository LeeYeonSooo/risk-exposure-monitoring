...
function canonProto(otherId: string): string {
  return otherId.replace(/^protocol:/, "").replace(/^dl:/, "").split("@")[0].replace(/_/g, "-").toLowerCase();
}
...
export function applyConcentricLayout(opts: {
  ...
  const poolsFor = (chain: string, otherId: string) => poolsByKey.get(`${chain}|${canonProto(otherId)}`) ?? [];
