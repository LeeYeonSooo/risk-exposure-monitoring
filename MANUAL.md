...
### 2026-06-14 — 마켓 자체오라클 자동화(전 수용처) + 상류 배킹 공유노드(EigenLayer·Ethena) + Veda 재예치 + 엣지 오라클 툴팁
관계맵 오라클 표시를 "자체 오라클 쓰는 모든 수용처"로 일반화 + 배킹(상류·하류) 의존성 추가. 전부 온체인/실데이터 검증, automation·frontend typecheck GREEN. (git 없음.)
- **Morpho 오라클 lego 자동화(원래 갭)**: 관계맵은 메인 어댑터(`adapters/morpho-blue.ts`)와 **별개 경로**(`lego/discover.ts`→`lego/acceptors.ts`→`/api/lego`→`lego-overlay.ts`)인데 GraphQL에서 `oracleAddress`를 안 가져와 Morpho 마켓 엣지에 오라클 원이 없었다. 수정: discover GraphQL에 `oracleAddress` 추가 → `AcceptingMarket.oracleAddress` → snapshot-lego 후처리 ⑥''가 기존 `introspectOracle` 재사용해 `meta.oracle`(종류)·oracleProvider·oracleDescription 채움(규모순 정렬·캐시·상한). 프론트는 GraphCanvas가 이미 읽는 `omkt.oracle`로 무변경 렌더. 검증: weETH 13 교환비 + 3 시장가(전부 verified, 실 피드설명 "weETH/eETH Exchange Rate"·"Redstone").
- **B2 — 오라클 일반화(수용처 5종)**: acceptor가 `meta.oracleAddress`만 실으면 후처리가 자동 분류. 각 enumerator에 오라클 획득 배선(전부 온체인 검증): Euler `EVault.oracle()`→EulerRouter, Fluid API `oracle` 필드(0콜), Silo `SiloConfig.getConfig(silo).solvencyOracle`(word7, 실검증 `0xa26bdc88…`), Curve LlamaLend `amm().price_oracle_contract()`, Dolomite API `riskInfo.oracle`(체인 공유 중앙라우터). Euler/LlamaLend/Dolomite는 라우터가 불투명(description/feed/Etherscan 이름 신호 없음)→심볼 휴리스틱 폴백(**verified=false 정직 표기**), Morpho·Fluid 일부는 온체인 분류(verified). 관계맵 오라클 원 48→91개. Aave는 풀형(PoolAddressesProvider 경유 필요)이라 **§18 백로그**(메인 맵엔 이미 오라클 있음).
- **C4 — 엣지 오라클 툴팁**: 마켓 노드 meta에만 있던 오라클을 collateral_at 엣지 `attrs.oracle`로도 전파(`lego-overlay.oracleAttr`) → 원(circle)과 HoverTooltip "Oracle" 행(provider/description)이 일치.
- **C5 — introspect 상한 24→36**: 다중 수용처가 oracleAddress 싣기 시작하며 토큰당 고유 오라클 증가 대비(규모순 유지, introspectOracle 전역 주소캐시라 누적은 "전체 고유 오라클 수"로 수렴).
- **B3 — Veda BoringVault 수용처(`vedaAccepting`)**: 토큰이 배킹으로 재예치되는 일드 볼트. 온체인 레지스트리 없음 → 큐레이트 볼트 리스트(liquidETH·liquidUSD·UltraYield·eBTC) + `balanceOf(vault)` (>$1k 더스트컷). 검증: liquidETH가 weETH **$2.37M** 보유(나머지 더스트 정확 필터). 중요도 중(2차 — 공유노드 거의 안 만듦, silo).
- **B1 — 상류 배킹/언더라잉 체인(EigenLayer·Ethena)**: 토큰이 '무엇으로 구성되는가'를 언더라잉 토큰→기반자산→리스테이킹 venue 까지 **단일 노드 체인**(`backed_by` relation, cyan)으로. `UNDERLYING_CHAIN` 레지스트리 + snapshot-lego ⑥'''. 예: **weETH→eETH→ETH→EigenLayer**(온체인 검증: `weETH.eETH()`=eETH rate 1.097, eETH←1.84M ETH ether.fi LiquidityPool), **sUSDe→USDe→Ethena**(`sUSDe.asset()==USDe`). 기반(ETH)·venue id 체인 공유라 형제 토큰(weETH·rsETH·ezETH)이 같은 ETH·EigenLayer 노드로 모임. **프론트**: 새 `backed_by` relation(types/edge-colors/FloatingEdge cyan)·`lego-overlay` 단일노드 체인 렌더(프로토콜 껍데기 안 만듦 → 이전 "EigenLayer 노드 2개 중복" 버그 해소)·`tree-layout`이 체인을 토큰 **위로** 곧게 세움(언더라잉=상류). venue TVL=DeFiLlama($4.70B/$4.84B). 멤버십: EigenLayer는 **정적 정설 LRT 리스트**(온체인 토큰 프로브 불가 — `StrategyManager.stakerStrategyListLength(LRT)=0`, 리스테이킹은 프로토콜별 NodeDelegator/EigenPod가 함 — 연구 검증). **흐름맵**(별도): flow-core에 배킹을 의사 프로토콜로 누적 → weETH·rsETH가 한 `proto:ethereum|eigenlayer`, sUSDe·USDe가 한 `proto:ethereum|ethena`로 묶임(상류 공동위험).
- **배킹 의존성 중요도(온체인 정량, 2026-06-14)**: 공유노드 규모 = EigenLayer $4.16B(weETH·ezETH·rsETH 공유) · Ethena USDe $4.84B(sUSDe·USDe) ≫ 큐레이터 볼트 수렴 $91.5M(Sentora PYUSD가 weETH $49.8M+sUSDe $41.7M 동시 펀딩) ≫ Veda 재예치 $3.6M. 결론: **자산-배킹 레이어(EigenLayer·Ethena)가 맵 최대 숨은 공유노드** — yield-볼트 재예치보다 한 자릿수 큼. Curve 멀티-LRT 풀은 TVL ~$1K로 死(정직한 부정).
...
---
**진행 현황**: ✅ 완료 — P0-1·P0-2·P1-4·P1-5·P2-6·P2-7·P2-8 (+ ouroboros 신호 제거). 2026-06-13 보완(§17): 수용처 레지스트리+Aave/볼트쉐어/Fluid 어댑터·Euler 멀티체인·token_variants(Base 래핑본·CCIP 1급화)·래핑본 레고 연결·추정 브릿지 토글·가중 PageRank. 2026-06-14 보완(§17): 마켓 자체오라클 자동화(Morpho+Euler/Fluid/Silo/LlamaLend/Dolomite)·엣지 오라클 툴팁·상류 배킹 공유노드(EigenLayer·Ethena, 관계맵+흐름맵)·Veda 재예치 수용처. 🔜 남음 — P1-3(IRM 곡선) · 레고가 viewed 베이스 토큰도 수용처 질의에 투입(Fluid 0건 해소) · 추정만-있는 토큰 브릿지 토글 노출 · ccip_remote 셀렉터→체인명 매핑 · P3(deferred).

**2026-06-14 신규 백로그(오라클·배킹 후속)**:
- **Aave-lego 오라클**: lego 관계맵 Aave 리저브 엣지엔 오라클 원 없음(메인 프로토콜 맵엔 있음). `aaveAccepting`에 `PoolAddressesProvider.getPriceOracle()`→`AaveOracle.getSourceOfAsset(collateral)` 배선하면 자동 표시(주소 provider 플러밍 필요).
- **불투명 라우터 오라클 해상도 개선**: Euler(EulerRouter)·Curve LlamaLend(price_oracle wrapper)·Dolomite(중앙 라우터)는 현재 휴리스틱 폴백(verified=false). EulerRouter `resolveOracle(base,quote)`·LlamaLend Chainlink 하위피드까지 해상하면 실분류(verified=true) 가능.
- **흐름맵 배킹 누락(공급 0)**: flow-core 배킹 contrib = 토큰 온체인 공급 USD. `fetchChainSupplies`가 0 반환하는 토큰(예: ezETH 일부 런)은 배킹노드 미표시 — 공급 폴백(시총/DeFiLlama) 보강 후순위.
- **Veda 볼트 리스트 확장**: 온체인 팩토리 부재로 큐레이트 4개만(liquidETH·liquidUSD·UltraYield·eBTC). Veda 공개 디렉터리(오프체인) 동기화로 확장 가능.
